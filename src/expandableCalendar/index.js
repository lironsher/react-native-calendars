...
class ExpandableCalendar extends Component {
	...
	constructor(props) {
		...
		this._headerStyles = {
			style: {
				top:
					this.props.initialPosition === POSITIONS.CLOSED
						? 0
						: -HEADER_HEIGHT,
			},
		};
		...
		if (this.props.horizontal) {
			...
			if (this.state.position === POSITIONS.OPEN) {
				...
				const firstDayOfWeek =
					(next ? 7 : -7) - dayOfTheWeek + firstDay;
		...
		return (
			<View
				style={[
					...
					{
						paddingLeft:
							_.get(this.props, 'calendarStyle.paddingLeft') +
								6 || DAY_NAMES_PADDING,
						paddingRight:
							_.get(this.props, 'calendarStyle.paddingRight') +
								6 || DAY_NAMES_PADDING,
					},
				]}
			>
