...
export default class AgendaView extends Component {
	...
	constructor(props) {
		...
		this.headerHeight = this.props.showWeekHeader ? HEADER_HEIGHT : 1;
		this.state = {
	...
	onScrollPadLayout() {
		...
		this.setScrollPadPosition(
			this.props.isDefaultViewCalendar
				? 0
	...
	onLayout(event) {
		...
		if (this.props.isDefaultViewCalendar) {
			this.calendar.scrollToDay(
				this.state.selectedDay.clone().setDate(1),
				0,
				false
			);
		} else {
	...
	onSnapAfterDrag(e) {
		...
		this.openScrollPadToY(currentY);
	}
	openScrollPadToY(currentY) {
	...
	render() {
		...
		const weekdaysStyle = [
			this.styles.weekdays,
			{
				opacity: this.state.scrollY.interpolate({
					inputRange: [
						agendaHeight - this.headerHeight,
						agendaHeight,
					],
					outputRange: [0, 1],
					extrapolate: 'clamp',
				...
				transform: [
					{
						translateY: this.state.scrollY.interpolate({
							inputRange: [
								Math.max(0, agendaHeight - this.headerHeight),
								agendaHeight,
							],
		...
		const scrollPadPosition =
			(shouldAllowDragging ? this.headerHeight : 0) - KNOB_HEIGHT;
				...
				<Animated.View style={headerStyle}>
					...
					>
						<CalendarList
							...
							theme={this.props.theme}
							onVisibleMonthsChange={this.onVisibleMonthsChange.bind(
								this
							)}
