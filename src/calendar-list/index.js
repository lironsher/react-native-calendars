...
class CalendarList extends Component {
	...
	constructor(props) {
		...
		this.onViewableItemsChangedBound = this.onViewableItemsChanged.bind(
			this
		);
	...
	scrollToDay(d, offset, animated) {
		...
		let scrollAmount =
			size * this.props.pastScrollRange +
			diffMonths * size +
			(offset || 0);
	...
	scrollToMonth(m) {
		...
		const size = this.props.horizontal
			...
			: this.props.calendarHeight;
		const scrollAmount =
			size * this.props.pastScrollRange + diffMonths * size;
	...
	componentWillReceiveProps(props) {
		...
		if (
			nextCurrent &&
			current &&
			nextCurrent.getTime() !== current.getTime()
		) {
		...
		for (let i = 0; i < rowclone.length; i++) {
			...
			if (rowclone[i].getTime) {
				...
				val.propbump = rowclone[i].propbump
					? rowclone[i].propbump + 1
					: 1;
			}
			newrows.push(val);
		}
	...
	onViewableItemsChanged({ viewableItems }) {
		function rowIsCloseToViewable(index, distance) {
			for (let i = 0; i < viewableItems.length; i++) {
				if (
					Math.abs(index - parseInt(viewableItems[i].index)) <=
					distance
				) {
					return true;
				}
	...
	addMonth = count => {
		this.updateMonth(
			this.state.currentMonth.clone().addMonths(count, true)
		);
	};
